...
`npm run eval:frontend:ru` runs the frozen Cyrillic slice against `fixtures/next-react-ts`; mixed Cyrillic/Latin prompts are part of the Russian slice. `npm run release:check` runs build, syntax checks, all tests, registry validation/lint/audit, the full 157-prompt routing suite, and this Russian gate. For repeated OpenCode comparisons, keep the same fixture, frozen Russian task slice, `opencode` target, installed versions/checksums, and at least three repetitions for each separately labeled GLM and DeepSeek model. External-model results are analytical evidence and do not block deterministic local build/test gates.
