...
The frontend pack is SkillRanger's reference domain implementation. Release `0.4.0` owns frontend routing policy, structured design artifacts, eight product recipes, 18 versioned design rules across six families, eight worked example packs, 80 deterministic rendered assets, deterministic validation rules, workflows, and frontend eval slices. The package identity and published contract are recorded in [`domain.manifest.json`](domain.manifest.json) and [`release.json`](release.json); the visual benchmark suite is pinned to `0.4.0`.
