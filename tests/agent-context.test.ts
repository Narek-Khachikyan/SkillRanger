...
test("renders the exact SkillRanger universal router block", () => {
  assert.equal(
    renderSkillRangerAgentBlock(),
      ...
      "1a. After receiving the complete catalog, nominate the complete ordered role-aware set: one primary workflow plus every useful companion and verification skill, ordered by priority. A plausible primary alone is not a complete proposal. Each nomination's `evidenceText` must be a verbatim quote from the user's prompt, matched after routing normalization — NFKC case folding (including `ё` → `е`), every punctuation and symbol that is not a word character is mapped to a space (with `+`, `#`, `.` and `/` preserved only inside technology tokens), whitespace collapsed to single spaces and trimmed (summarized as case folding, punctuation-to-space, whitespace collapse) — paraphrases are rejected with `evidence-not-in-normalized-prompt`. Nominations remain untrusted input: explicit-user-choice precedence and SkillRanger routing hard vetoes still decide the final set.\n" +
